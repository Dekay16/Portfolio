...
namespace Portfolio.Business.Managers
{
    public class AdminManager : IAdminManager
    {
        ...
        public List<TrafficSummaryViewModel> GetTrafficSummary(string range, DateTime? startDate, DateTime? endDate)
        {
            ...
            {
                _logger.LogError(ex, "Error in AdminManager.GetTrafficSummary");
                throw;
            }
        }
        ...
        public List<TrafficLog> GetTrafficLogs(DateTime? startDate, DateTime? endDate)
        {
            ...
            {
                _logger.LogError(ex, "Error in AdminManager.GetTrafficLogs");
        ...
        public List<ErrorLog> GetErrorLogs(DateTime? startDate, DateTime? endDate)
        {
            try
            {
                var query = _db.Errors.AsQueryable();
            ...
            {
                _logger.LogError(ex, "Error in AdminManager.GetErrorLogs");
                return new List<ErrorLog>();
            }
        }

        public List<TrafficSummaryViewModel> GetErrorSummary(DateTime? startDate, DateTime? endDate)
        {
            try
            {
                var query = _db.Errors.AsQueryable();

                DateTime start = startDate?.Date ?? DateTime.UtcNow.Date.AddDays(-29);
                DateTime end = endDate?.Date ?? DateTime.UtcNow.Date;

                var grouped = query
                    .Where(e => e.TimeStamp.Date >= start.Date && e.TimeStamp.Date <= end.Date)
            ...
            {
                _logger.LogError(ex, "Error in AdminManager.GetErrorSummary");
                return new List<TrafficSummaryViewModel>();
            }
        }
